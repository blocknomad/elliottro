...
export default function drawPointerLines(
  ...
  const y = Math.floor(event.clientY - rect.top) + 0.5;
  ...
  if (x < klinesWidth && y < klinesHeight) {
    ...
    const xpos =
      klineWidth / 2 +
      klineWidth * klineIndex +
      (klineWidth / 2) * (klineIndex + 1);
    ...
    context.fillText(
      `O:${kline.open.toFixed(8)}  H:${kline.high.toFixed(
        8
      )}  L:${kline.low.toFixed(8)}  C:${kline.close.toFixed(8)}`,
      3,
      13,
      klinesWidth
    );
    ...
    const boxHeight = 19;
    const rectYOffset = Lodash.clamp(
      y - boxHeight / 2,
      0,
      klinesHeight - boxHeight
    );
    ...
    context.fillText(
      xText,
      rectXOffset + boxPadding / 2,
      klinesHeight + 1 + boxHeight / 2
    );
  }
