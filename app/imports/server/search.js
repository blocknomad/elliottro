...
Meteor.methods({
  screen({
    ...
    quoteAssets,
    range = 100,
    chart = { type: "reversal", pattern: "HST" },
  }) {
    ...
    Lodash.forEach(data, ({ quoteAsset, baseAsset, exchange, klines }) => {
      // Slice klines range
      ...
      for (let wSize = pattern.series.length; wSize <= input.length; wSize++) {
    ...
    return {
      matches: Lodash.sortBy(matches, ["quoteAsset", "exchange", "baseAsset"]),
